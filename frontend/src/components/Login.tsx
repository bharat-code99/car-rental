...
export default function Login({ setShowLogin }: LoginProps) {
  ...
  return (
    <div onClick={() => setShowLogin(false)} className="fixed top-0 left-0 right-0 bottom-0 z-100 flex justify-center items-center text-sm text-gray-600 bg-black/80">
      <form
      ...
      >
        ...
        <div className="flex items-center mt-4 w-full bg-white border border-gray-300/80 h-12 rounded-md overflow-hidden pl-6 gap-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="14"
            height="14"
        ...
        <button
          type="submit"
          className="mt-2 w-full h-11 rounded-md text-white bg-primary hover:opacity-90 transition-opacity"
        >
          {state === "login" ? "Login" : "Sign up"}
        </button>
        <p
